...
<html lang="en">
  ...
  <div id="pcoded" class="pcoded">
				                                        ...
				                                        <form class="form-material" enctype="multipart/form-data" action="<%= request.getContextPath() %>/ServletUsuarioController" method="post" id="formUser">
                                                            ...
                                                            <div class="form-group form-default input-group mb-4">
                                                            	<div class="input-group-prepend">
                                                            		<c:if test="${modelLogin.foto != null && modelLogin.foto != ''}">
                                                            			<a href="<%= request.getContextPath() %>/ServletUsuarioController?acao=downloadFoto&id=${modelLogin.id}">
                                                            				<img alt="Foto" id="fotoBase64" src="${modelLogin.foto}" width="70px" />
                                                            			</a>
                                                            		</c:if>
                                                            		<c:if test="${modelLogin.foto == null || modelLogin.foto == ''}">
                                                            			<img alt="Foto" id="fotoBase64" src="assets/images/user.png" width="70px" />
                                                            		</c:if>
                                                            	</div>
                                                            	<input type="file" id="fileFoto" name="fileFoto" accept="image/*" onchange="visualizarFoto('fotoBase64', 'fileFoto');" class="form-control-file" style="margin-top: 15px; margin-left: 5px;"/>
                                                            </div>
                                                            <div class="form-group form-default form-static-label">
                                                            ...
                                                            <div class="form-group form-default form-static-label">
                                                                <input type="text" name="logradouro" id="logradouro" class="form-control" required="required" value="${modelLogin.endereco.logradouro}">
                                                                <span class="form-bar"></span>
                                                                <label class="float-label">Logradouro (Rua):</label>
                                                            </div> 
                                                            <div class="form-group form-default form-static-label">
                                                                <input type="text" name="bairro" id="bairro" class="form-control" required="required" value="${modelLogin.endereco.bairro}">
                                                                <span class="form-bar"></span>
                                                                <label class="float-label">Bairro:</label>
                                                            </div> 
                                                            <div class="form-group form-default form-static-label">
                                                                <input type="text" name="localidade" id="localidade" class="form-control" required="required" value="${modelLogin.endereco.localidade}">
                                                                <span class="form-bar"></span>
                                                                <label class="float-label">Localidade (Cidade):</label>
                                                            </div> 
                                                            <div class="form-group form-default form-static-label">
                                                                <input type="text" name="uf" id="uf" class="form-control" required="required" value="${modelLogin.endereco.uf}">
                                                                <span class="form-bar"></span>
                                                                <label class="float-label">UF (Estado):</label>
                                                            </div> 
                                                            <div class="form-group form-default form-static-label">
                                                                <input type="text" name="numero" id="numero" class="form-control" required="required" value="${modelLogin.endereco.numero}">
                                                                <span class="form-bar"></span>
												            ...
												            <c:if test="${modelLogin.id > 0}">
												            	<a href="<%= request.getContextPath() %>/ServletTelefoneController?idUsuario=${modelLogin.id}" class="btn btn-primary waves-effect waves-light">Telefone</a>
												            </c:if>
											  ...
											  <tbody>
											  	<c:forEach items="${usuarios}" var="usuario"> 
											  		<tr>
											  			...
											  			<td><a class="btn btn-success" href="<%= request.getContextPath() %>/ServletUsuarioController?acao=buscarEditar&id=${usuario.id}">Ver</a></td>
											  		</tr>
											  	</c:forEach>
											  </tbody>
											</table>
    ...
    <script type="text/javascript">
        ...
        function visualizarFoto(fotoBase64, fileFoto) {
        	var preview = document.getElementById(fotoBase64); // img
        	var file = document.getElementById(fileFoto).files[0]; // input file
    	...
    	function buscarUsuario() {
 			...
 			if (nomeBusca != null && nomeBusca != '' && nomeBusca.trim() != '') {
 				...
 				$.ajax({
 					method: "get",
 					url: urlAction,
 					data: "nomeBusca=" + nomeBusca + '&acao=buscarUsuario-ajax',
 					success: function(response, textStatus, xhr) {
 						...
 						for (var p = 0; p < totalPagina; p++) {
 							var url = 'nomeBusca=' + nomeBusca + '&acao=buscarUsuario-ajax-page&pagina=' + (p * 5);
 							$("#ulPaginacaoUsuarioAjax").append('<li class="page-item"><a class="page-link" href="#" onclick="buscaUserPageAjax(\''+ url +'\')">' + (p + 1) + '</a></li>');
 						}
 					}
 				}).fail(function(xhr, status, errorThrown) {
